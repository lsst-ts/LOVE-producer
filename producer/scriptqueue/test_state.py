...
class TestScriptqueueState(asynctest.TestCase):
    ...
    async def test_state(self):
        ...
        for duration in durations:
            ...
            self.scripts_remotes[index] = salobj.Remote(
        ...
        stream = await asyncio.wait_for(
            self.wait_for_script_state_to_match(index, "current", "RUNNING", "RUNNING"),
            timeout=LONG_TIMEOUT,
        )
        ...
        # current script
        expected_script = await asyncio.wait_for(
            self.make_expected_script(self.scripts_remotes[current_index]), LONG_TIMEOUT
        )
        ...
        for script_index in waiting_indices:
            expected_script = await asyncio.wait_for(
                self.make_expected_script(self.scripts_remotes[script_index]),
                LONG_TIMEOUT,
            )
            produced_script = next(
                s for s in stream["waiting_scripts"] if s["index"] == script_index
            )
        ...
        for script_index in finished_indices:
            ...
            produced_script = next(
                s for s in stream["finished_scripts"] if s["index"] == script_index
            )
            self.assertEqual(expected_script, produced_script)
