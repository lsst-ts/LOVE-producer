...
class EventsProducer:
    ...
    async def send_initial_state_data(self):
        ...
        initial_state_data_keys = [
            key
            for key in self.initial_state_data.keys()
            if self.initial_state_data[key] is not None
        ]

        self.log.debug(f"Sending initial data for {initial_state_data_keys}.")

        for key in initial_state_data_keys:
            (csc, salindex, evt_name) = key

            evt_data = self.initial_state_data.pop(key)

            if evt_name == "heartbeat":
                continue
